...
export const useChatSocketAuth = (conversationId: string | null) => {
  ...
  const [messages, setMessages] = useState<Message[]>([{ speakerId: 0, text: introText }])
  ...
  const connectWebSocket = useCallback(() => {
    ...
    ws.onmessage = (event: MessageEvent<string>) => {
      ...
      if (data.type === 'conversation_id') {
        ...
        const historyMessages: Message[] = [{ speakerId: 0, text: introText }]
        ...
        if (text === '<start>') {
          ...
          addMessage(0, chunkAccumulator.current)
          setChunk([])
          chunkAccumulator.current = ''
        } else {
          addChunk(text)
  ...
  const resetChat = useCallback(() => {
    setMessages([{ speakerId: 0, text: introText }])
    setChunk([])
    setInputText('')
    chunkAccumulator.current = ''
  }, [])
