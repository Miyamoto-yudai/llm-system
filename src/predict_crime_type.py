...
def answer(hist, add_optional_questions=True):
    ...
    if not "MOVE" in dicision:
        print("does not move")
        response_text = dicision

        # 任意の追加質問を生成
        if add_optional_questions:
            import src.chat as chat
            if chat.optional_follow_up_manager.should_add_optional_questions(hist, {"type": "predict_crime_type"}):
                optional_questions = chat.optional_follow_up_manager.generate_optional_questions(
                    hist,
                    {"type": "predict_crime_type"},
                    response_text
                )
                if optional_questions:
                    return response_text + optional_questions
